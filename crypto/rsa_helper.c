...
{
	KFTF_EXPECT_NOT_NULL(rsa_parse_pub_key_arg, key);
	// bound the maximum size of the key that we accept
	if (arg->key_len > 16 * PAGE_SIZE)
		return;
...
{
	KFTF_EXPECT_NOT_NULL(rsa_parse_priv_key_arg, key);
	// bound the maximum size of the key that we accept
	if (arg->key_len > 16 * PAGE_SIZE)
		return;
