...
FUZZ_TEST(test_bitmap_parselist, struct bitmap_parselist_arg)
{
	...
	KFUZZTEST_EXPECT_NOT_NULL(bitmap_parselist_arg, buf);
	KFUZZTEST_EXPECT_IN_RANGE(bitmap_parselist_arg, nmaskbits, 16,
				  1024 * 10);
	KFUZZTEST_ANNOTATE_STRING(bitmap_parselist_arg, buf);
