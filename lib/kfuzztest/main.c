 ...
 * under "/sys/kernel/debug/kfuzztest/<test-name>" containing one write-only
 * "input" file used for receiving inputs from userspace.
 *
...
static int __init kfuzztest_init(void)
{
	...
	/* Taint the kernel as the debugfs interface allows userspace to run
	 * almost arbitrary kernel code, which is a major security risk. */
